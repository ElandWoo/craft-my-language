...
public class SimpleParser {
    ...
    private SimpleASTNode intDeclare(TokenReader tokens) throws Exception {
        SimpleASTNode node = null;
        Token token = tokens.peek();
        if (token != null && token.getType() == TokenType.INTTK) {
            token = tokens.read();
    ...
    private SimpleASTNode additive(TokenReader tokens) throws Exception {
        ...
        if (child1 != null) {
            while (true) {                              //循环应用add'规则
                Token token = tokens.peek();
                if (token != null && (token.getType() == TokenType.PLUS || token.getType() == TokenType.MINU)) {
    ...
    private SimpleASTNode multiplicative(TokenReader tokens) throws Exception {
        ...
        while (true) {
            Token token = tokens.peek();
            if (token != null && (token.getType() == TokenType.MULT || token.getType() == TokenType.DIV)) {
                token = tokens.read();
    ...
    private SimpleASTNode primary(TokenReader tokens) throws Exception {
        ...
        if (token != null) {
            if (token.getType() == TokenType.INTCON) {
                ...
                token = tokens.read();
                node = new SimpleASTNode(ASTNodeType.IDENFR, token.getText());
            } else if (token.getType() == TokenType.LPARENT) {
                tokens.read();
                node = additive(tokens);
                if (node != null) {
                    token = tokens.peek();
                    if (token != null && token.getType() == TokenType.RPARENT) {
                        tokens.read();
                    } else {
